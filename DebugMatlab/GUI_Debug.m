...
handles.t=0;
handles.length = 100;
handles.x=0;%zeros(1,handles.length);                 %Initialize x
handles.y=zeros(1,handles.length);                    %Initialize y array
handles.z=0;                                          %Initialize z

handles.s=0;

handles.osc = plot(handles.axes_osc, (1:handles.length), handles.y(1:handles.length));
plot(handles.axes_tot, 0, 0);

%handles.tmr = timer('TimerFcn',{@TmrFcn,handles.figure1},'Period',0.01,'ExecutionMode','FixedRate');
...
if(handles.s.BytesAvailable > 0)
    ...
    a = fscanf(handles.s, '%8f %8f');   %Read from serial

    if numel(a) == 2                    %If input data is correct
    
        handles.x(handles.t+1) = a(1);
        handles.y(handles.t+1) = a(2);
        handles.z(handles.t+1) = a(2);

        if handles.t >= handles.length
           disp_y = handles.y(handles.t-handles.length+(1:handles.length));
        else
           disp_y = handles.y(1:handles.length);
        end 

        set(handles.osc, 'Ydata', disp_y);
        plot(handles.axes_tot, handles.x, handles.z);
...
if state == 1                       %If start
    set(handles.edit_length, 'Enable', 'off');      %Disable the edit_lenth function
    set(handles.edit_length, 'String', handles.length);
    set(handles.update_length, 'Enable', 'off');      %Disable the edit_lenth function
...
% handles    structure with handles and user data (see GUIDATA)
stop(handles.tmr);          %Stop Timer
fwrite(handles.s,2);       %Send -1 (reset) on Serial Port
flushinput(handles.s);      %Flushing Input Buffer
set(handles.edit_length, 'Enable', 'on');       %Enable the edit_length function
set(handles.update_length, 'Enable', 'on');       %Enable the edit_length function
set(handles.togglebutton_OnOff, 'Value', 0);    %Set Run/Stop button to Stop
handles.t = 0;                                  %Reset time
handles.x = 0;%zeros(1,handles.length);         %Reset x
handles.y = zeros(1,handles.length);            %Reset y
handles.z = 0;                                  %Reset z
handles.osc = plot(handles.axes_osc, (1:handles.length), handles.y);    %Prepare first graph
plot(handles.axes_tot, 0, 0);                                           %Prepare second graph
guidata(hObject, handles);
